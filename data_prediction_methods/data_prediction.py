...
def prepare_data(scrape_file):
    ...
    df_sorted["team_code"] = df_sorted["Team_Name"].astype(
        "category").cat.codes
    df_sorted["hour"] = df_sorted["Start (ET)"].str.extract(
        r'(\d+)')[0].astype(int)
...
def add_rolling_averages(df, window=10):
    cols = ["FG", "FGA", "FG%", "3P", "3PA", "3P%", "2P", "2PA", "2P%", "FT",
    ...
    df = df.groupby(["Team_Name", "Season"],
                    group_keys=False).apply(caclulate_rolling)
...
def main():
    ...
    df = prepare_data("stat_scraping/nba_scrape_data.csv")
    ...
    results, accuracy, precision, rf = train_and_evaluate(
        df, all_predictors, test_date="2025-04-13")
